...
| **gRPC Server** | 暴露 `StartGame` / `StopGame` / `SessionStatus` / `GeneratePlayerToken` / `UpdatePortMapping` / `PauseGame` / `ResumeGame` / `SaveState` / `LoadState` 等 RPC，供 Control Plane 调用 |
...
Worker.StartGame(room_id, rom_path, rom_url, emulator_type, max_ports):
  ...
  4. 从 rom_url（MinIO 预签名 URL）下载 ROM 文件到本地:
   ...
   5. 启动 EmuRunner 子进程（附带 stdin/stdout 管道）:
      exec.Command("emurunner",
        "--publisher-host", livekitHost,
        "--token", token,
        "--room", roomID,
        "--rom", "/tmp/cloudemu/{room_id}/rom.dat",
        "--backend", emulatorType)
      → stdin/stdout 通过 os.Pipe() 连接，用于 Worker↔EmuRunner 控制命令通信
      → stderr 接入 Worker 日志
...
Worker.StopGame(room_id):
  1. 关闭 stdin/stdout 管道（通知 EmuRunner 子进程退出）
  2. 找到对应 EmuRunner 子进程 → SIGTERM
  3. 超时 5s 未退出 → SIGKILL
  4. 删除 LiveKit 房间
  5. 清理临时工作目录: rm -rf /tmp/cloudemu/{room_id}/
  6. 更新 Redis 心跳中的 sessions 计数
```
...
EmuRunner 只能被动接收指令，无 DB / 对象存储能力，且房间关闭后即销毁。
存档数据经 **Worker 中转 + 共享目录**：EmuRunner 与 Worker 同主机，共享 `/tmp/cloudemu/{room_id}/`。
**控制指令已从 LiveKit DataChannel 迁移至 stdin 管道**（见 8.8 节），消除了文件轮询。
...
房主点「存档」
  ...
  → Worker.SaveState:
       1. 通过 stdin 管道发送 {"cmd":"save_state"} → 等待 stdout 响应 {"cmd":"save_state","status":"ok"}
       2. 从共享目录读 state.dat → HTTP PUT 到 MinIO 预签名 URL → 返回 size
  → EmuRunner 收到 save_state 命令: retro_serialize → 写 state.dat → 回复响应
...
**存档管理**：房主还可对存档进行以下操作（均需房主身份）：

- **改名** `POST /api/rooms/rename-save-state {room_id, save_state_id, name}`：仅改 DB `name` 字段，存档创建时默认名为时间戳。
- **删除** `POST /api/rooms/delete-save-state {room_id, save_state_id}`：先删 MinIO 对象（失败仅告警不阻断），再删 DB 记录。
- **加载最新存档** `POST /api/rooms/load-latest-state {room_id}`：取当前机种+ROM 的最新一条存档，走与读档相同的 Worker 反序列化流程，省去列表选择。
...
房主选存档点「读取」
  ...
  → Worker.LoadState:
       1. HTTP GET 下载状态二进制到共享目录 load.dat
       2. 通过 stdin 管道发送 {"cmd":"load_state"} → 等待 stdout 响应 {"cmd":"load_state","status":"ok"}
  → EmuRunner 收到 load_state 命令: 读 load.dat → retro_unserialize → 回复响应
```
...
| 决策 | 选择 | 理由 |
|------|------|------|
| EmuRunner↔Worker 控制通道 | **stdin/stdout 管道** | 双向可靠，毫秒级延迟，无需 LiveKit 中转 |
| EmuRunner↔外部 状态传输 | 共享文件 + Worker 中转 | EmuRunner 无 DB/对象存储能力，复用 Worker 的网络能力与同主机共享目录 |
| 完成回执 | **管道 JSON 响应** | 替代原有的共享目录标志文件轮询，零新增网络链路 |
| 每房间存档数 | 多存档槽（时间戳列表，可改名/删除） | 房主可回溯任意历史存档，并管理命名与清理 |
| 加载最新存档 | 单独 load-latest-state 接口 | 常用场景一键读档，无需先拉列表再选择 |
...
### 8.8 Worker ↔ EmuRunner 管道通信

控制消息（pause/resume/port_map/save_state/load_state）已从 LiveKit DataChannel 迁移至 stdin/stdout 管道。

```
Worker ──[stdin: JSONL]──→ EmuRunner 子进程
Worker ←──[stdout: JSONL]── EmuRunner 子进程
```

**管道创建**（`process.go`）：`os.Pipe()` 创建 stdin/stdout 管道 → `cmd.Stdin = reader` / `cmd.Stdout = writer`，子进程启动后 Worker 侧关闭子进程持有的 FD 端。

**协议格式**（`emurunner/protocol.go`）：JSONL，每条命令一行，响应按序匹配：

```json
// 命令
{"cmd":"pause"}
{"cmd":"port_map","mapping":{"0":"player:uuid1"}}
{"cmd":"save_state"}

// 响应
{"cmd":"pause","status":"ok"}
{"cmd":"save_state","status":"ok","size":12345}
```

**串行化**：`Session.cmdMu` 互斥锁保证同一时刻只有一个命令在执行，防止 stdout 响应交错。

**管道关闭**（`closePipes`）：Stop 时先关闭 `cmdIn`（stdin writer），EmuRunner 的 `bufio.Scanner` 检测到 EOF 后退出命令读取协程；再关闭 `cmdOutFile`（stdout reader）；最后 SIGTERM 终止子进程。

**保留在 DataChannel 的协议**：

| 包类型 | 用途 | 保留原因 |
|--------|------|----------|
| 0x01 | 手柄输入 | 60Hz 实时按键，延迟敏感 |
| 0x03/0x04 | ping/pong | 端到端延迟探测，须走 WebRTC |
